...
const sendNoticeEmail = async ({
  to, parentName, studentName, noticeTitle, content, postedBy, postedAt, attachments = [],
}) => {
  const dateLabel = new Date(postedAt || Date.now()).toLocaleDateString('en-IN', { dateStyle: 'long' })
  ...
  const noticeBox = [
    '<div style="padding:20px 24px;background:#f5f3ff;border-left:4px solid #3b33c4;border-radius:8px;margin:0 0 24px;">',
  ...
  const emailContent = [
    ...
    '<p style="font-size:15px;color:#6b7280;line-height:1.7;margin:0 0 24px;">A new notice has been posted for <strong style="color:#111827;">' + studentName + '</strong>.</p>',
    noticeBox,
    '<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin:0 0 4px;">',
    td('Posted By', postedBy),
    td('Date', dateLabel),
    '</table>',
    attachmentsSection,
  ].join('\n')
